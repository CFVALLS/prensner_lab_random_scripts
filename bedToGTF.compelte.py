...
def process_bed_line(input_line: list, df_in: pd.DataFrame) -> None:
    ...
    try:
        ...
        attributes = 'gene_id "{}"; transcript_id "{}"; orf_id "{}"; gene_name "{}"; gene_type "{}"'.format(input_line[3], 'transcript_' + input_line[3], input_line[3], input_line[18], input_line[16])
        
        if block_nums <= 1:
            start = int(input_line[1]) + 1  # BED is 0-based; GTF is 1-based
            end = int(input_line[2])
            attributes = attributes + '; exon_number 1'
            df_in.loc[len(df_in)] = [chrom, source, "gene", start, end, score, strand, frame, attributes] 
            df_in.loc[len(df_in)] = [chrom, source, "transcript", start, end, score, strand, frame, attributes]
            df_in.loc[len(df_in)] = [chrom, source, "CDS", start, end, score, strand, frame, attributes]
            df_in.loc[len(df_in)] = [chrom, source, "exon", start, end, score, strand, frame, attributes]
            ...
            for size, start in zip(block_sizes, block_starts):
                ...
                gene_end = new_end
                df_in.loc[len(df_in)] = [chrom, source, "CDS", new_start, new_end, score, strand, frame, attributes + '; exon_number {}'.format(exon_id)]
                df_in.loc[len(df_in)] = [chrom, source, "exon", new_start, new_end, score, strand, frame, attributes + '; exon_number {}'.format(exon_id)]
                loop_count += 1
            df_in.loc[len(df_in)] = [chrom, source, "gene", gene_start, gene_end, score, strand, frame, attributes + '; exon_number {}'.format(exon_id)]
            df_in.loc[len(df_in)] = [chrom, source, "transcript", gene_start, gene_end, score, strand, frame, attributes + '; exon_number {}'.format(exon_id)]
