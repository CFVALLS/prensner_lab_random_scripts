...
def process_bed_line(input_line: list, df_in: pd.DataFrame) -> None:
    ...
    try:
        ...
        if block_nums <= 1:
            ...
            df_in.loc[len(df_in)] = [chrom, source, "CDS", start, end, score, strand, 0, attributes]
            ...
            for size, start in zip(block_sizes, block_starts):
                ...
                gene_end = new_end
                df_in.loc[len(df_in)] = [chrom, source, "CDS", new_start, new_end, score, strand, 0, attributes + '; exon_number {}'.format(exon_id)]
